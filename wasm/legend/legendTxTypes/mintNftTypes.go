...
func (txInfo *MintNftTxInfo) VerifySignature(pubKey string) error {
	...
	inner, err := abiEncoder.Pack("", messageTypeBytes32, txInfo.CreatorAccountIndex, txInfo.ToAccountIndex, txInfo.ToAccountNameHash, txInfo.NftContentHash, txInfo.NftCollectionId, txInfo.CreatorTreasuryRate, txInfo.GasAccountIndex, txInfo.GasFeeAssetId, txInfo.GasFeeAssetAmount, txInfo.ExpiredAt, txInfo.Nonce, ChainId)
	if err != nil {
		return err
	}
