...
func (txInfo *WithdrawNftTxInfo) VerifySignature(pubKey string) error {
	...
	inner, err := abiEncoder.Pack("", messageTypeBytes32, txInfo.AccountIndex, txInfo.NftIndex, txInfo.ToAddress, txInfo.GasAccountIndex, txInfo.GasFeeAssetId, txInfo.GasFeeAssetAmount, txInfo.ExpiredAt, txInfo.Nonce, ChainId)
	if err != nil {
		return err
	}
