...
func (txInfo *TransferTxInfo) VerifySignature(pubKey string) error {
	...
	inner, err := abiTransfer.Pack("", messageTypeBytes32, txInfo.FromAccountIndex, txInfo.ToAccountIndex, txInfo.ToAccountNameHash, txInfo.AssetId, txInfo.AssetAmount, txInfo.GasAccountIndex, txInfo.GasFeeAssetId, txInfo.GasFeeAssetAmount, txInfo.ExpiredAt, txInfo.Nonce, ChainId)
	if err != nil {
		return err
	}
