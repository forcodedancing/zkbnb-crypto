...
func (txInfo *TransferNftTxInfo) VerifySignature(pubKey string) error {
	...
	inner, err := abiEncoder.Pack("", messageTypeBytes32, txInfo.FromAccountIndex, txInfo.ToAccountIndex, txInfo.ToAccountNameHash, txInfo.NftIndex, txInfo.CallDataHash, txInfo.GasAccountIndex, txInfo.GasFeeAssetId, txInfo.GasFeeAssetAmount, txInfo.ExpiredAt, txInfo.Nonce, ChainId)
	if err != nil {
		return err
	}
